...
int main( int argc, char **argv )
{
    ...
    while (( opt = getopt_long( argc, argv, "dhp:v", gLongOption, NULL )) > 0 )
    {
        switch ( opt )
        {
            case 'd':
    ...
    gPortFd = open( devName, O_RDWR | O_NOCTTY | O_SYNC );
    if (gPortFd < 0)
    {
    ...
    *
    ...
    while(0)
    {
	    fprintf(stderr, "write something to the port %s\n", devName);

	    write(gPortFd, "d 1\n", 4);	// disable pulses
	    sleep(5);

	    write(gPortFd, "m 1 1000 20000\n", 15);  // configure pulses
	    write(gPortFd, "e 1\n", 4); // enable pulses again
	    sleep(2);

	    fprintf(stderr, "ask for help from the port\n");
	    write(gPortFd, "?\n", 2);
	    sleep(1);

	    fprintf(stderr, "read response from the port\n");

	    for( i=0; i<10; i++)
	    {
	    	len = read(gPortFd,buf,255);
	    	if (len > 0)
    ...
    */
    ...
    // Unblock the termination signals so the user can kill us if we hang
